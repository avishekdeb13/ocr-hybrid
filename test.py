...
def uploadFileToS3(folderName,filename,extension):
    try:
        ...
        response = s3_client.upload_file(
            "./{folderName}/{filename}_masked.pdf".format(folderName=folderName, filename=filename),
            "{OUTPUT_BUCKET}".format(OUTPUT_BUCKET=OUTPUT_BUCKET), "{filename}_masked.pdf".format(filename = filename))
        print("File Uploaded")
        return True
...
def convertImagesToPdf(folderName,outputFolder,filename):
    ...
    for image in os.listdir(folderName):
        ...
        img = Image.open("./{folderName}/{image}".format(folderName=folderName, image = image))
        #print(f"img:{img}")
        images.append(img)
    ...
    convertedImages[0].save("./{outputFolder}/{filename}_masked.pdf".format(outputFolder=outputFolder,filename=filename), save_all=True, append_images=convertedImages[1:])
# convertImagesToPdf("output","output")

# FOR CONVERTING PDF INTO IMAGES
def convertPdftoImages(inputPath,outputPath,fileName):
    pages = convert_from_path("./{inputPath}/{fileName}.pdf".format(inputPath=inputPath,fileName=fileName))

    isFolderExist(outputPath)

    for i in range(len(pages)):
        pages[i].save('./'+outputPath+'/{fileName}_'.format(fileName=fileName)+ str(i) +'.jpg', 'JPEG')

def deleteFile(foldername,filename):
    os.remove("./{foldername}/{filename}".format(foldername=foldername,filename=filename))
    #print(f"file with {filename} is deleted!")

def deleteFilesFromFolder(foldername):
    for image in os.listdir(foldername):
        os.remove("./{foldername}/{image}".format(foldername=foldername,image=image))


def detectTextFromImage(folderName,imageName):
    with open('./{folderName}/{imageName}'.format(folderName=folderName,imageName=imageName), 'rb') as img:
...
def maskImage(folerName,outputFolder,imageName,textDetections):
    ...
    image = Image.open("./{folerName}/{imageName}".format(folerName=folerName,imageName=imageName))
    regex = ("^[2-9]{1}[0-9]{3}\\" +
             "s[0-9]{4}\\s[0-9]{4}$")
    p = re.compile(regex)
    ...
    image.save("./{outputFolder}/masked_{imageName}".format(outputFolder=outputFolder,imageName=imageName))
...
def skewCorrectionInFolder(foldername):
    for image in os.listdir(foldername):
        ...
        img.save("./{foldername}/{image}".format(foldername=foldername,image=image))
...
def main():
    try:
        for obj in bucket.objects.all():
            ...
            # keyname (e.g = Xyz.pdf)
            keyname = obj.key
            print("keyname:{keyname}".format(keyname=keyname))
        ...
        print("Something went wrong:{error}".format(error=error))
